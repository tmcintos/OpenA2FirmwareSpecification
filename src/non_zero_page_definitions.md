...
| $07FB | CURSOR | Cursor type and status. | Defines the type of cursor displayed by input routines (e.g., block, checkerboard). |
| $047B | OLDCH | Old Horizontal Cursor Position. | Stores the previous horizontal cursor position; used in 80-column mode to track cursor movement. |
| $057B | OURCH | Our Horizontal Cursor Position. | Stores the current horizontal cursor position in 80-column mode; used internally by 80-column display routines. |
| $067B | VFACTV | Video Firmware Active Flag. | Bit 7 = 0 when video firmware is inactive; used to test if 80-column card is active. |
