...
ROUTINE_FILES = $(sort $(filter-out $(TITLE_PAGE) $(INTRODUCTION) $(SUMMARY) $(MEMORY_LOCATIONS) $(ZERO_PAGE_DEFS) $(NON_ZERO_PAGE_DEFS) $(BOOT_SEQUENCE_OVERVIEW) $(MONITOR_USER_INTERFACE) $(PERIPHERAL_ROM_HEADING) $(BOOT_ROM_IDENTIFICATION) $(PERIPHERAL_ROM_SECTION) $(DISK_ROM_DETAILED), $(wildcard $(SRC_DIR)/*.md)))
